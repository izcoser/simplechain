...
class Transaction:
    ...
    def __init__(self, _fr: bytes, _to: bytes, _amount: int, _signature: bytes):
        self.fr = _fr
        self.to = _to
        self.amount = _amount
    ...
    def verify_signature(self):
        try:
            ...
            message = self.fr + self.to + to_bytes(self.amount)
            vk.verify(self.signature, message)
            return True
        except BadSignatureError:
            return False

def create_transaction(pk: bytes, to: bytes, amount: int) -> Transaction:
    fr = address(pk)
    message = fr + to + to_bytes(amount)
    signature = SigningKey.from_string(pk, SECP256k1).sign(message)
    return Transaction(fr, to, amount, signature)

def mine_block(state: State, transactions: list[Transaction]):
    for t in transactions:
        if t.amount <= state.balances[t.fr] and t.verify_signature():
...
def test_signatures():
    ...
    tx = create_transaction(pk, to, 100)
